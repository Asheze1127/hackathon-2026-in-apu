...
| チーム体制     | win(front), take(back), nakashima(UI), hianta(AI)で行なう |
| 予算制約      | 最低限で運用する方針                                            |
...
| ツリー可視化             | Prettier + ESLint + Husky (Pre-commit hook) | 意思決定ツリーのノード・エッジ描画に特化したライブラリ                                |
| Linter / Formatter | Prettier + ESLint + Husky (Pre-commit hook) | コードの一貫性を保つ / 設定が軽量                                         |
...
| API     | Next.js Route Handlersまたはserver actions | フロントと同一リポジトリで完結 / サーバーサイド処理を簡潔に書ける              |
| ORM     | Prisma                                  | TypeScript フレンドリー / Supabase（PostgreSQL）との相性が良い |
| 認証      | Supabase Auth                           | メール認証 / OAuth をすぐに使える / JWT 発行も組み込み             |
| AI 呼び出し | OpenRouter                              | OpenRouter 経由で LLM を呼び出す                        |
